    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for key in newPosDist.keys():
                dist[key] += self.beliefs[oldPos] * newPosDist[key]
        self.beliefs = dist
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for key in dist.keys():
            dist[key] *= self.getObservationProb(observation, gameState.getPacmanPosition(), key, self.getJailPosition())
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        dist = self.getBeliefDistribution()
        for particle in dist.keys():
            for i in range(self.numGhosts):
                dist[particle] *= self.getObservationProb(observation[i], \
                                    gameState.getPacmanPosition(), particle[i], self.getJailPosition(i))
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [dist.sample() for i in range(self.numParticles)]
