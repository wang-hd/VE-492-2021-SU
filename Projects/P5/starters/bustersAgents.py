    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        mostLikelyPos = [key.argMax() for key in livingGhostPositionDistributions]
        closestPos = None
        ...
        for action in legal:
            succDistance = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestPos)
            if succDistance < optDistance:
                optAction = action
                optDistance = succDistance
        return optAction